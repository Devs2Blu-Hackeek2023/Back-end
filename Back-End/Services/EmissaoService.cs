...
namespace Back_End.Services
{
	public class EmissaoService : IEmissaoService
	{
		...
		public EmissaoService(DataContext context, CameraService cameraService)
		{
			_context = context;
			_cameraService = cameraService;
		}

		private async Task<EmissaoModel> IniciarEmissao()
		{
			RuaModel? rua = await _cameraService.SortearRuaAsync();
			VeiculoModel? veiculo = await _cameraService.SortearVeiculoAsync();

			if (veiculo is null || rua is null) return null;
			else
			{
				_veiculosAtuais.Add(veiculo);
				EmissaoModel inicioEmissao = new EmissaoModel()
				{
					Rua = rua,
				...
				return inicioEmissao;
			}
		}

		//private async Task<EmissaoModel> FinalizarEmissao()
		//{
